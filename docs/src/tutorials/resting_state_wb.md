...
```@tutorial resting-state-circuit
using Neuroblox
using CSV
using DataFrames
using MetaGraphs
...
sol = solve(prob, EulerHeun(), dt=0.5)
...
```
Fig. Left: Connection Adjacency Matrix, Right: log10(p value) displaying statistally significant correlation between time series
