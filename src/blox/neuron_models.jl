...
mutable struct HHNeuronBlox <: NeuronBlox
	E_syn::Num
    G_syn::Num
	I_in::Num
	freq::Num
    ...
    connector::Num
    odesystem::ODESystem
	function HHNeuronBlox(;name,E_syn=0.0,G_syn=2,I_in=0,freq=0,phase=0,τ=10,nrntype="excitatory")
	        
		sts = @variables V(t)=-65.00 n(t)=0.32 m(t)=0.05 h(t)=0.59 Isyn(t)=0.0 G(t)=0.0 z(t)=0.0 jcn(t)=0.0
		if nrntype=="excitatory"
		    ...
		    ps = @parameters E_syn=E_syn G_Na = 52 G_K  = 20 G_L = 0.1 E_Na = 55 E_K = -90 E_L = -60 G_syn = G_syn V_shift = 0 V_range = 35 τ_syn = 10 τ₁ = 0.1 τ₂ = τ τ₃ = 2000 I_in=I_in freq = freq phase = phase
		...
		new(E_syn, G_syn, I_in, freq, phase, τ, nrntype, odesys.G, odesys)
	end
end	
