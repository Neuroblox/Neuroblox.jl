...
mutable struct HarmonicOscillatorBlox{T} <: NeuralMassBlox
    ...
    function HarmonicOscillatorBlox(;name, ω=25*(2*pi), ζ=1.0, k=625*(2*pi), h=35.0)
        para_dict = scope_dict!(Dict{Symbol,T}(:ω => ω,:ζ => ζ,:k => k,:h => h))
        ω=para_dict[:ω]
        ζ=para_dict[:ζ]
        k=para_dict[:k]
        h=para_dict[:h]
        ...
        new{typeof(para_dict)}(para_dict, odesys.x,[odesys.x],[odesys.x,odesys.y],
...
mutable struct JansenRitCBlox{T} <: NeuralMassBlox
    ...
    function JansenRitCBlox(;name, τ=0.001, H=20.0, λ=5.0, r=0.15)
        para_dict = scope_dict!(Dict{Symbol,T}(:τ => τ,:H => H,:λ => λ,:r => r))
        τ=para_dict[:τ]
        H=para_dict[:H]
        λ=para_dict[:λ]
        r=para_dict[:r]
