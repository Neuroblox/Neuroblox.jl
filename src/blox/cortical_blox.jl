...
function cortical_blox(;name, nblocks=6, blocksize=6)
    ...
    function synaptic_network(;name, sys=sys, adj_matrix=adj_matrix, input_ar=input_ar,inh_nrn = inh_nrn, inh_mod_nrn = inh_mod_nrn)
        ...
        for ii = 1:length(sys)
            ...
            if inh_mod_nrn[ii]>0
                eq2 = [0 ~ postsyn_nrn.Iasc - input_ar];
                push!(syn_eqs,eq2[1])
            end
    ...
    syn_net = synaptic_network(name=name,sys=nrn_network,adj_matrix=syn, input_ar=asc_input, inh_nrn = inh_nrn, inh_mod_nrn=inh_mod_nrn)
    return syn_net, syn
end
...
function cortical_blox_new(;name, nblocks=6, blocksize=6)
...
mutable struct CorticalBloxNew <: NeuralMassBlox
    # all parameters are Num as to allow symbolic expressions
    connector::Num # symbolic expression to connect with another blox
    output::Vector{Num} # list of symbols that we want to output for plotting
    mean::Vector{Num} # list of strings that we will take the mean over for plotting
    adjM::Array{Float64,2} # adjacency matrix
    odesystem::ODESystem
    function CorticalBloxNew(;name,nblocks=20, blocksize=6)
        odesys, adjm = cortical_blox_new(name=name,nblocks=nblocks, blocksize=blocksize)
        statesV = [s for s in states.((odesys,), states(odesys)) if contains(string(s),"V(t)")]
        new(sum(statesV)/length(statesV), statesV, statesV, adjm, odesys)
    end
end
