...
AdjacencyMatrix(blox::AbstractComposite) = AdjacencyMatrix(get_connector(blox))
...
"""
    adjmatrixfromdigraph(g::MetaDiGraph)

Construct the weighted adjacency matrix of a directed graph.
"""
...
Given an adjacency matrix, populate the graph `g` with the edges stored in 
the adjacency matrix. The `connection_rule` keyword argument dictates the 
type of connection. Possible values for this argument include the following:
    - "basic": simple weighted connection
    - "psp": postsynaptic potential connection
"""
