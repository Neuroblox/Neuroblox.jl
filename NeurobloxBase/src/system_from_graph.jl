...
"""
    get_blox_by_name(g::MetaDiGraph, name::Union{String, Symbol})

Return the blox with `name` if it exists in the graph `g` and return nothing otherwise.
"""
function get_blox_by_name(g::MetaDiGraph, name::Union{String, Symbol})
    for v in vertices(g)
        b = get_prop(g, v, :blox)
...
function add_edge!(g::MetaDiGraph, p::Pair{A, B}; kwargs...) where {A <: Union{AbstractObserver, AbstractBlox}, B <: Union{AbstractObserver, AbstractBlox}}
    src, dest = p
...
function generate_discrete_callbacks(blox::AbstractNeuron, bc::Connector; t_block = missing)
    sa = spike_affects(bc)
...
function system_from_graph(g::MetaDiGraph, p::Vector{Num}=Num[]; name=nothing, t_block=missing, simplify=true, graphdynamics=false, kwargs...)
    if graphdynamics
        system_from_graph(to_graphsystem(g), p; graphdynamics = true, kwargs...)
    else
        if isnothing(name)
...
function system_from_graph(g::GraphSystem, p=Num[]; graphdynamics=false, kwargs...)
    if !graphdynamics
...
"""
    learning_rules_from_graph(g::MetaDiGraph)

Return a dictionary that maps the connection weight parameters of the graph to their learning rules, 
which dictate how they will change as the model evolves.
"""
