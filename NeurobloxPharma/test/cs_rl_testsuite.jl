using NeurobloxPharma, GraphDynamics, Test, Random, OrdinaryDiffEqVerner, OhMyThreads, Statistics
...
function big_corticostriatal_learning_run(; t_block = 90, # ms (size of discrete time blocks)
    ...
    @time begin
        ...
        @named ITN = NextGenerationEI(;namespace=global_ns, Cₑ=2*36,Cᵢ=1*36, Δₑ=0.5, Δᵢ=0.5, η_0ₑ=10.0, η_0ᵢ=0.0, v_synₑₑ=10.0, v_synₑᵢ=-10.0, v_synᵢₑ=10.0, v_synᵢᵢ=-10.0, alpha_invₑₑ=10.0/36, alpha_invₑᵢ=0.8/36, alpha_invᵢₑ=10.0/36, alpha_invᵢᵢ=0.8/36, kₑₑ=0.0*36, kₑᵢ=0.6*36, kᵢₑ=0.6*36, kᵢᵢ=0*36) 
	    ...
	    @named tan_nrn = HHNeuronExci(;namespace=global_ns) 
        ...
        @named tan_pop1 = TAN(;namespace=global_ns, rng=rrng()) 
        @named tan_pop2 = TAN(;namespace=global_ns, rng=rrng()) 
