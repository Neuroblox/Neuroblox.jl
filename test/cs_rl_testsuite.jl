using Neuroblox, GraphDynamics, Test, Random, OrdinaryDiffEq, OhMyThreads
...
function small_corticostriatal_learning_run(;time_block_dur = 90.0, ## ms (size of discrete time blocks)
    ...
    @time begin
        image_set = CSV.read(
            joinpath(@__DIR__(), "../examples/smaller_cs_stimuli_set.csv"), DataFrame) ## reading data into DataFrame format
        model_name=:g
        ...
        add_connection!(g, VAC => AC, weight=3, density=0.1, learning_rule = hebbian_cort)
        add_connection!(g, AC => STR1, weight = 0.075, density =  0.04, learning_rule =  hebbian_mod, rng=Xoshiro(1234))
        add_connection!(g, AC => STR2, weight =  0.075, density =  0.04, learning_rule =  hebbian_mod)
        
        add_connection!(g, tan_pop1 => STR1, weight = 1, t_event = time_block_dur)
        add_connection!(g, tan_pop2 => STR2, weight = 1, t_event = time_block_dur)
        add_connection!(g, STR1 => tan_pop1, weight = 1)
        add_connection!(g, STR2 => tan_pop1, weight = 1)
        add_connection!(g, STR1 => tan_pop2, weight = 1)
        add_connection!(g, STR2 => tan_pop2, weight = 1)
        add_connection!(g, STR1 => STR2, weight = 1, t_event = 2*time_block_dur)
        add_connection!(g, STR2 => STR1, weight = 1, t_event = 2*time_block_dur)
        
        add_connection!(g, STR1 => SNcb, weight = 1)
        add_connection!(g, STR2 => SNcb, weight = 1)
...
function big_corticostriatal_learning_run(; t_block = 90, # ms (size of discrete time blocks)
    ...
    @time begin
	    ...
	    @named VC = CorticalBlox(N_wta=45, N_exci=5,  density=0.01, weight=1,I_bg_ar=0;namespace=global_ns, rng=rrng()) 

        @named PFC = CorticalBlox(N_wta=20, N_exci=5, density=0.01, weight=1,I_bg_ar=0;namespace=global_ns, rng=rrng())
	    ...
	    add_connection!(g, VC => PFC; weight = 1, density = 0.08, learning_rule = hebbian_cort, rng=rrng()) #VC->pfc
	    add_connection!(g, PFC => STR1; weight = 0.075, density = 0.04, learning_rule = hebbian_mod, rng=rrng()) #pfc->str1
	    add_connection!(g, PFC => STR2; weight = 0.075, density = 0.04, learning_rule = hebbian_mod, rng=rrng()) #pfc->str2
    ...
    @time run_experiment!(agent, env; alg=Vern7(), t_warmup, save_everystep=false, verbose=true)
end
