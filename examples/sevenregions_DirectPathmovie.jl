...
# ╔═╡ cdb26fd3-2e38-485d-8f33-dafea506748d
# Make the animation

begin
	hsa = 0.01
	animg = @animate while hsa<500
		# set the parameters for the simulation using the sliders
		p_an = prob.p
		p_an[gpeh_idx] = hg
		p_an[stnh_idx] = hsa
		...
		p_lfpa = plot(sol_an.t, sol_an[1,:],label="GPe", lw=2.5, xlims=(0,1), color="blue", fg_legend = :false)
		p_lfpa = plot!(sol_an.t, sol_an[3,:],label="STN H=$(@sprintf("%.2f", hsa))", lw=2.5, xlims=(0,5), xlabel="time in sec", ylabel="arb. V", color="orange")
		title!("Simulated LFP")
	...
	gif(animg, "anim_fps15_direct.gif", fps = 15)
end
